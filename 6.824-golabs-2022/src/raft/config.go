...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	...
	if index != -1 && index != lastIncludedIndex {
		err := fmt.Sprintf("server %v snapshot doesn't match m.SnapshotIndex, index: %v, lastIncludedIndex: %v", i, index, lastIncludedIndex)
		return err
	}
	cfg.logs[i] = map[int]interface{}{}
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			if retry == false {
				log.Panicf("one(%v) failed to reach agreement1", cmd)
			}
		} else {
	...
	if cfg.checkFinished() == false {
		log.Panicf("one(%v) failed to reach agreement2", cmd)
	}
	return -1
}
